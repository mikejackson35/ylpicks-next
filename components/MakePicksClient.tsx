...
export default function MakePicksClient() {
  ...
  if (!tournament) return <p className="text-slate-500">No upcoming tournament available for picks.</p>;
  ...
  if (tournament.locked) return (
    <div className="max-w-md md:max-w-xl">
      <div className="mb-5 md:mb-8">
        <h2 className="text-2xl md:text-3xl font-bold text-slate-900">Make Picks</h2>
        <p className="text-sm md:text-base text-slate-500 mt-1">{tournament.name}</p>
      </div>
      <div className="bg-slate-100 rounded-xl border border-slate-200 overflow-hidden">
        {[1,2,3,4,5,6].map((tier) => {
          ...
          return (
            <div key={tier} className="flex items-center gap-4 px-4 py-4 md:py-5 border-b border-slate-200 last:border-0">
              <div className="flex items-center gap-2.5 w-14 shrink-0">
                <div className={`w-4 h-4 rounded-full ${TIER_DOT[tier]}`} />
                <span className="text-sm md:text-base font-medium text-slate-500">T{tier}</span>
              </div>
              <span className={`text-sm md:text-base ${selected ? "font-medium text-slate-900" : "text-amber-400"}`}>
    ...
    <div className="max-w-md md:max-w-xl">
      <div className="mb-5 md:mb-8 flex items-center justify-between md:block">
        ...
        <button
          ...
          className="md:hidden bg-emerald-600 text-slate-900 rounded-xl px-5 py-2.5 text-base font-semibold hover:bg-emerald-600 disabled:opacity-40 transition-colors"
        >
          {saving ? "Saving..." : "Save"}
        </button>
      </div>

      <div className="bg-slate-100 rounded-xl border border-slate-200 overflow-hidden mb-5 md:mb-6">
        {[1,2,3,4,5,6].map((tier) => {
          ...
          return (
            <div key={tier} className="flex items-center gap-4 px-4 py-4 md:py-5 border-b border-slate-200 last:border-0">
              ...
              {players.length === 0 ? (
                <p className="text-sm md:text-base text-slate-400">No players assigned</p>
              ) : (
                <select
                  value={sel}
                  onChange={(e) => setPicks((prev) => ({ ...prev, [tier]: e.target.value }))}
                  className="flex-1 bg-white border border-slate-300 rounded-lg px-3 py-2.5 md:py-3 text-sm md:text-base text-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-600"
                >
      ...
      <button
        onClick={handleSave}
        disabled={saving || missing.length > 0}
        className="hidden md:block bg-emerald-600 text-slate-900 rounded-lg px-8 py-3 text-base font-semibold hover:bg-emerald-600 disabled:opacity-40 transition-colors"
      >
      ...
      {message && (
        <p className={`mt-3 text-sm px-3 py-2 rounded-lg border ${
          message.type === "success"
            ? "text-emerald-600 bg-emerald-950/50 border-emerald-800"
            : "text-red-600 bg-red-950/50 border-red-800"
        }`}>
          {message.text}
        </p>
      )}
